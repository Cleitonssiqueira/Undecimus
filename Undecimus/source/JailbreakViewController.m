...
int inject_persistent_dylib(const char *file, const char *dylib) {
    ...
    const char *insert_dylib_args[] = { "insert_dylib", "--all-yes", "--inplace", "--overwrite", dylib, file, NULL };
    if (insert_dylib_main(6, insert_dylib_args) != ERR_SUCCESS) {
        rv = -4;
        goto out;
    }
    if (runCommand("/usr/libexec/ldid", "-M", "-S", file, NULL) != ERR_SUCCESS) {
        rv = -5;
        goto out;
    }
    if (injectTrustCache(@[@(file)], GETOFFSET(trustcache), pmap_load_trust_cache) != ERR_SUCCESS) {
        rv = -6;
        goto out;
    }
